package com.ezenbooks.backend.jwt;
...
import com.ezenbooks.backend.security.service.PrincipalDetails;
...
//         }
         ...
         UserDTO userDTO = om.readValue(request.getInputStream(), UserDTO.class); // 로그인을하면 username, password 값을 User.class객체에 담아 user에 보내줌
         System.out.printf("username:%s password:%s\n", userDTO.getUser_name(), userDTO.getUser_pwd());

         UsernamePasswordAuthenticationToken authenticationToken =
        		 new UsernamePasswordAuthenticationToken(userDTO.getUser_name(), userDTO.getUser_pwd());

         Authentication authentication = authManager.authenticate(authenticationToken); // 인증이 처리되는 단계

         PrincipalDetails principalDetails = (PrincipalDetails) authentication.getPrincipal();
         System.out.printf("로그인 	완료 됨(인증)  %s %s %"
         		+ "s ", principalDetails.getUsername(), principalDetails.getPassword(), principalDetails.getUsernickname());
   ...
   protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
      ...
      String jwtToken = JWT.create()
            ...
            .withClaim("authRole", principalDetails.getUser().getUser_role()) // 권한
...
=======
      ...
      body.put("userrole", principalDetails.getUser().getUser_role());
